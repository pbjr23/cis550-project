  ...
  oracle.connect(connectData, function(err, connection) {
    if ( err ) {
    	console.log(err);
	  	...
	  	// selecting rows
	  	connection.execute("SELECT * FROM address WHERE rownum <= 10", 
	  			   [], 
...
function getUserAddress(String username, callback) {
	oracle.connect(connectData, function(err, connection) {
		if (err) {
			console.log(err);
		} else {
...
function getUserLatLong(String username, callback) {
	oracle.connect(connectData, function(err, connection) {
		if (err) {
			console.log(err);
		} else {
